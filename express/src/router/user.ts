...
export function userRouter() {
    ...
    router.delete("/api/user", async (req, res) => {
        ...
        if (!authRes) {
            logger("WARN", "Delete user failed: Unauthorized");
            res.status(401).json({ error: "Unauthorized" });
            return;
        }
        ...
        const result = await tryCatch(db.delete(users).where(eq(users.id, user.id)));
    ...
    router.patch("/api/user", async (req, res) => {
        logger("INFO", "PATCH /api/user called");

        const authRes = await auth(req);

        if (!authRes) {
            logger("WARN", "Patch user failed: Unauthorized");
            res.status(401).json({ error: "Unauthorized" });
            return;
        }
        ...
        const body = bodySchema.safeParse(req.body);
        if (!body.success) {
            logger("WARN", `Validation failed during update: ${JSON.stringify(body.error.errors)}`);
            res.status(400).json({ error: body.error.errors });
            return;
        }
        ...
        logger("DEBUG", `Updating user: ${user.email} with fields: ${JSON.stringify(Object.keys(updates))}`);

        const result = await tryCatch(db.update(users).set(updates).where(eq(users.id, user.id)));
